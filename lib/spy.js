...
function handleCircular(store) {
  return _jsan["default"].parse(_jsan["default"].stringify(store, null, null, {
    circular: '[CIRCULAR]'
  }));
}

function init(store, config) {
  var name = config.name || mobx.getDebugName(store);
  configure(name, config);
  stores[name] = store;
  var devTools = (0, _remotedev.connectViaExtension)(config);
  devTools.subscribe((0, _monitorActions.dispatchMonitorAction)(handleCircular(stores[name]), devTools, onlyActions[name]));
...
function schedule(name, action) {
  ...
  if (action && !(0, _filters.isFiltered)(action, filters[name])) {
    toSend = function toSend() {
      monitors[name].send(action, handleCircular(stores[name]));
    };
  }
