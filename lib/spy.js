...
function filterStore(store, config) {
  return (0, _lodash["default"])(store, function (it) {
    return Object.assign(it, _defineProperty({}, config.storeFilter, 'circular'));
  });
}
...
function init(store, config) {
  ...
  devTools.subscribe((0, _monitorActions.dispatchMonitorAction)(stores[name], devTools, onlyActions[name]));
